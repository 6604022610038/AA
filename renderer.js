...
function displayMessage(msgData) {
    ...
    if (msgData.type === 'message') {
        ...
        // [FIXED] ตรวจสอบ Base64 string format
...
function renderJoinedRooms() {
    ...
    Object.keys(joinedRooms).forEach(roomName => {
        ...
        roomDiv.addEventListener('click', () => {
            // [FIXED: ส่งรหัสเป็นค่าว่าง]
            // เมื่อคลิกสลับห้อง (Auto-rejoin) ให้ส่งรหัสผ่านเป็นค่าว่าง
...
// [FIXED] แยก Logic การส่งค่ารหัสผ่านเมื่อสร้าง/เข้าร่วมครั้งแรก
function handleRoomAction(roomName, roomPassword = '', isNewRoom = false) {
    ...
    // Server จะตรวจสอบรหัสและจัดการ Join
    ws.send(JSON.stringify({ 
        type: 'createRoom', // ใช้ 'createRoom' เพื่อให้ Server ตรวจสอบการสร้าง/เข้าร่วม
        room: roomName,
        password: roomPassword, // ส่งรหัสผ่านเฉพาะเมื่อเป็นการสร้าง/เข้าร่วมครั้งแรก (หรือสลับห้องถ้า Server ต้องการ)
        isNewRoom: isNewRoom
    }));
}
...
ws.onmessage = (event) => {
    ...
    if(data.type === 'message' || data.type === 'file') {
        // [FIXED] แสดงข้อความเมื่ออยู่ในห้องปัจจุบันเท่านั้น และไม่ใช่ข้อความของตัวเอง (เพื่อป้องกันการแสดงซ้ำ)
...
if (joinRoomBtn) {
    joinRoomBtn.addEventListener('click', () => {
        ...
        // [MODIFIED] ส่งรหัสผ่านที่ผู้ใช้กรอกมา เพื่อเข้าร่วมครั้งแรก
...
if (createRoomBtn) {
    createRoomBtn.addEventListener('click', () => {
        ...
        // [MODIFIED] ส่งรหัสผ่านที่ผู้ใช้กรอกมา เพื่อสร้างห้อง
        handleRoomAction(room, password, true); // true = เป็นการสร้างห้องใหม่
