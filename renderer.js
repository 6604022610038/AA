...
// [NEW FUNCTION] ฟังก์ชันรีเซ็ต UI ทั้งหมดกลับไปหน้า Login (แทน window.location.reload())
function resetChatUI() {
    ...
    // **[FIXED]** ต้องแน่ใจว่ากลับไปที่โหมด Login เพื่อให้ปุ่ม Login ทำงาน
    setLoginMode(); 
}

// **[FIXED]** ฟังก์ชันตั้งค่าปุ่มสำหรับโหมด Login
function setLoginMode() {
    ...
    // ลบ Event Listeners ทั้งหมดก่อน
    ...
    // ตั้ง Event Listeners ที่ถูกต้องสำหรับโหมด Login
    loginBtn.addEventListener('click', submitLogin);
    registerBtn.addEventListener('click', switchToRegisterMode); 
}

// **[FIXED]** ฟังก์ชันตั้งค่าปุ่มสำหรับโหมด Register
function switchToRegisterMode() {
    ...
    loginBtn.style.display = 'block'; // เปลี่ยนให้ปุ่ม "เข้าสู่ระบบ" กลายเป็นปุ่ม "ย้อนกลับ"
    ...
    // ตั้ง Event Listeners ที่ถูกต้องสำหรับโหมด Register
    registerBtn.addEventListener('click', submitRegistration); 
    loginBtn.addEventListener('click', setLoginMode); // ปุ่มยกเลิก/ย้อนกลับ
}
...
function renderJoinedRooms() {
    ...
    Object.keys(joinedRooms).forEach(roomName => {
        ...
        roomDiv.addEventListener('click', () => {
            // เมื่อคลิกสลับห้อง ให้ส่งรหัสผ่านเป็นค่าว่าง
...
// **[แก้ไข]** ฟังก์ชันจัดการการกระทำในห้อง
...
ws.onmessage = (event) => {
    ...
    // ===== Logout Response =====
    if(data.type === 'logout') {
        // [FIXED] ใช้ฟังก์ชัน resetChatUI แทนการ reload
        localStorage.removeItem('chatUsername');
        resetChatUI(); 
    }
    
    // ===== Available Rooms / History / Messages (ส่วนที่ไม่มีการเปลี่ยนแปลง) =====
...
// **[FIXED]** Event Listener สำหรับปุ่ม Logout
if (logoutBtn) {
