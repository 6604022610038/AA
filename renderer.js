...
function handleRoomAction(roomName, roomPassword = '', isNewRoom = false) {
    ...
    const message = { 
        type: 'createRoom',
        room: roomName,
        isNewRoom: isNewRoom // ส่งค่า isNewRoom ไปให้ Server
    };

    // ถ้ามีการระบุรหัสผ่าน (ไม่ใช่ค่าว่าง) ให้ใส่ลงในข้อความ
    if (roomPassword) {
...
ws.onmessage = (event) => {
    ...
    // **[KEY FIX: NEW LOGIC FOR AVAILABLE ROOMS]**
    // รับรายชื่อห้องทั้งหมดที่มีจาก Server ทันทีหลัง Login เพื่อให้ห้องที่ไม่ต้องใส่รหัส (DD) ปรากฏ
