...
// rooms: { roomName: { password: 'xxx', users: [username, ...], history: [msgData, ...] } }
let users = {}; 
let clients = []; 
let rooms = {}; 
...
function broadcastRooms() {
    const publicRooms = Object.keys(rooms).filter(name => !rooms[name].password); // กรองเฉพาะห้องที่ไม่มีรหัส
...
function leaveRoom(ws) {
    if (ws.room && rooms[ws.room]) {
        ...
        // **[ลบห้องชั่วคราว]** ถ้าเป็นห้องที่ไม่มีรหัสผ่านและไม่มีผู้ใช้อยู่ อาจลบทิ้งได้
        // แต่ในโค้ดนี้ เราจะเก็บประวัติไว้เสมอแม้ห้องจะว่างเปล่า เพื่อให้ห้องส่วนตัวที่ถูกสร้างยังคงอยู่
...
function createOrJoinRoom(ws, roomName, roomPassword = '') {
    ...
    if(!rooms[roomName]) {
        // สร้างห้องใหม่
        rooms[roomName] = { 
            password: roomPassword, // รหัสจะถูกบันทึกไว้ ถ้าผู้ใช้ไม่ใส่ก็จะเป็น ''
            users: [], 
            history: [] 
        };
        saveData(); 
    ...
    // 5. **บันทึกห้องล่าสุด** และบันทึกข้อมูลลงดิสก์
    ...
    ws.on('message', (msg) => {
        ...
        if(data.type === 'login') {
            if(users[data.username] && users[data.username].password === data.password) {
                ...
                if (lastRoom && rooms[lastRoom]) {
                    // Try to auto-join the last room without a password (assuming it was successful last time)
        ...
        if((data.type === 'message' || data.type === 'file') && ws.username && ws.room) {
            const msgData = {
                ...
                // **[แก้ไข]** ใช้ Locale เพื่อให้ Timestamp เข้ากับ Client
