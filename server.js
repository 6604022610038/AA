...
function sendAvailableRooms(ws) {
    ...
    const userJoinedRooms = Object.keys(rooms).filter(roomName => {
        // เงื่อนไขการแสดงห้อง:
        // A. ห้องปัจจุบันที่กำลังเชื่อมต่ออยู่
        ...
        // C. (ทางเลือก) ห้องสาธารณะที่ควรเห็นทุกคน (ถ้ามี) - แต่จากโจทย์ใหม่คือไม่เอาห้องที่ยังไม่เคยเข้าเลย
...
function leaveRoom(ws) {
    ...
    ws.room = null;
    // ไม่ต้อง broadcast rooms ที่นี่ เพราะ Client จะต้องกรองเอง
}
    ...
    ws.on('message', (msg) => {
        ...
        if(data.type === 'login') {
            if(users[data.username] && (data.password === '' || users[data.username].password === data.password)) {
                ...
                // **[FIX] ส่งรายการห้องที่เคยเข้าเท่านั้น**
                sendAvailableRooms(ws);
